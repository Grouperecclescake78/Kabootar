...
# desugaring to run on our minSdk (21). The Android shell is regenerated by
# `flutter create` in CI, which produces a vanilla build.gradle.kts without it,
# so this re-applies the change after that step. Idempotent: safe to run on an
# already-patched file (e.g. a local checkout).
set -euo pipefail
